...
public class Feed_GetById_Tests
{
    ...
    [TestMethod]
    public async Task Get_all_unread_feed_articles_sorted_by_group()
    {
        await CreateTestUserFeedsAndArticlesAsync();

        Assert.AreEqual("", _webApplicationFactory.TestUser.AllUnreadSortOrder);

        using var client = await _webApplicationFactory.CreateAuthenticatedClientAsync();
        using var response = await client.GetAsync("/api/feed/-1?toggle=1");
        Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
        var responseContent = await response.Content.ReadFromJsonAsync<List<FeedArticle>>();
        Assert.AreEqual(3, responseContent?.Count);
        ...
        var userAccount = await services.ServiceProvider.GetRequiredService<IUserAccountRepository>().FindByEmailAsync(_webApplicationFactory.TestUser.Email);
        Assert.AreEqual("GroupAscThenDateAsc", userAccount?.AllUnreadSortOrder);
    }
